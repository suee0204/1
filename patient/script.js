...
document.getElementById("bookBtn").onclick=async()=>{const box=document.getElementById("bookingMsg");box.classList.add("hidden");const doctorId=document.getElementById("doctorSelect").value;const appointmentDateValue=document.getElementById("appointmentDate").value;const appointmentLocation=document.getElementById("location").value;const appointmentNotes=document.getElementById("notes").value;if(!doctorId){msg(box,"Please select a doctor.","error");return;}if(!appointmentDateValue){msg(box,"Please select an appointment date.","error");return;}if(!selectedTime){msg(box,"Please select an appointment time.","error");return;}if(!appointmentLocation){msg(box,"Please select a location.","error");return;}try{await api("/api/patient/appointments",{method:"POST",body:JSON.stringify({doctorId,appointmentDate:appointmentDateValue,appointmentTime:selectedTime,location:appointmentLocation,notes:appointmentNotes})});document.getElementById("successModal").classList.remove("hidden");await loadAppointments();}catch(err){msg(box,err.message,"error");}}
