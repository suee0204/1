...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        followUpPrescriptionId &&
        mongoose.Types.ObjectId.isValid(followUpPrescriptionId)
      ) {
        await Prescription.findOneAndUpdate(
          {
            _id: followUpPrescriptionId,
            patientId: req.session.userId,
            doctorId,
            followUpRequired: true,
            followUpBooked: false
        ...
        await writeAuditLog({
          ...
          metadata: {
            prescriptionId: followUpPrescriptionId,
...
app.post(
  "/api/admin/doctors/:id/reset-password",auth,requirePasswordChanged,role("admin"),async(req,res)=>{
    const temporaryPassword=String(req.body.password||"TempPassword1");
    if(temporaryPassword.length<10)return res.status(400).json({error:"Temporary password must have at least 10 characters."});
    if(!mongoose.Types.ObjectId.isValid(req.params.id))return res.status(404).json({error:"Doctor not found."});
    const doctor=await Account.findOneAndUpdate({_id:req.params.id,role:"doctor"},{passwordHash:await bcrypt.hash(temporaryPassword,12),mustChangePassword:true,passwordChangedAt:new Date()},{new:true});
    if(!doctor)return res.status(404).json({error:"Doctor not found."});
    await audit(req,{actorType:"admin",actorId:req.session.userId,action:"DOCTOR_PASSWORD_RESET",targetType:"Account",targetId:doctor._id,success:true});
    res.json({message:"Temporary password set. Doctor must change it at next sign-in.",temporaryPassword});
  }
);
